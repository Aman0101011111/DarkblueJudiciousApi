...
async def start_signup(channel):
    ...
    informal_role = discord.utils.get(channel.guild.roles, name="🔫 Informals")
    if informal_role:
...
async def add_strike(member, guild):
    ...
    # Handle 5 strikes
    if strike_count >= 5:
        informal_role = discord.utils.get(guild.roles, name="🔫 Informals")
...
async def on_message(message):
    ...
    if message.channel.id == 1378137911987404940:
        ...
        if signup_active and message.content.strip() == '+':
            informal_role = discord.utils.get(message.guild.roles, name="🔫 Informals")
